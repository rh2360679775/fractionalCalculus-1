...
inline double sinsum(double x,int terms) // sin(x) = x - x^3/3! + x^5/5! ...
{
	double term = x;    // first term of series
	double sum  = term; // sum of terms so far
	double x2   = x*x;
	for(int n = 1; n < terms; n++){
		term *= -x2 / (double)(2*n*(2*n+1)); // get next term from previous
...
int main(int argc,char *argv[])
{
	...
	for(int step = 0; step < steps; step++){
		double x = (double)( step_size*step );
