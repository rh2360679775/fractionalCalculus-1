...
//const double PI = acos(-1); // or std::numbers::pi in C++20
...
inline bool almostEqual(T a, T b) {
  ...
  inline double mittagLefflerENo( double alpha_new, double beta_new, double x ) {
  double result = 0;

  // ToDo: special cases: 
  if ( alpha_new > 1.0 ) {
    cout<< "case : alpha > 1, not yet implemented , returning NaN " <<endl;
    return numeric_limits<double>::quiet_NaN();   // not yet implemented
  }

  x = -x;  // internally with f(x) = MittagLefflerE(..., -x)
  
  SetAlphaBetaSolveEqsGetPsQs(alpha_new, beta_new);
  return getPadeApproximant(x,n,n);
}
